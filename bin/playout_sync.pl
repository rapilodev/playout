...
sub getHttp {
	...
	if ( $url =~ /(https?)\:\/\/([^\/\:]+)(\:(\d+))?\/(.+)/ ) {
		...
		Log::info("protocol=$protocol, domain=$domain, port=$port, path=/$path");
		local $/ = CRLF . CRLF;
		my $socket = IO::Socket::INET->new( PeerAddr => $domain, PeerPort => "http($port)" ) or die("cannot connect to $url");
		print $socket "GET /$path HTTP/1.0", CRLF, CRLF;
		my $header = <$socket>;
		print $header;
		print "#########################\n";
		my $data;
		print $data while read( $socket, $data, 60000 ) > 0;
		print "#########################\n";
	} else {
...
sub updateConfig {
	...
	$config->{log_level} = $verbose if defined $verbose;
	Log::setLevel( $config->{log_level} );
	MediaFiles::setMediaDir( $config->{mediaDir} );
	Time::setTimeZone( $config->{timeZone} );
	$syncSourceUrl = $config->{syncGetScheduleUrl} || '';
	Log::error( "cannot find syncGetScheduleUrl at " . Config::getConfigFile() ) if $syncSourceUrl eq '';
